...
> **Fork 说明：** 本仓库是 [jonntd/incipit](https://github.com/jonntd/incipit)，基于上游 [yc-duan/incipit](https://github.com/yc-duan/incipit) 的维护分支。请从本仓库安装——`npm install -g incipit` 装到的仍是上游 npm 包，不是这个 fork。
...
表面层（主题、工具卡、change-review）的 P0/P1/P2 规划与验收见 [`docs/ui-roadmap.md`](docs/ui-roadmap.md)。

## 安装

需要 Node.js 16 或更新版本，且本机已安装官方 Claude Code 扩展。

从 GitHub 安装**本 fork**（不是上游 npm 包）：

```bash
npm install -g github:jonntd/incipit
```

`incipit` 是终端 CLI，必须全局安装。`npm install github:jonntd/incipit`（不带 `-g`）是 npm 装本地依赖的写法，会把包丢进当前目录的 `node_modules/`，既不会注册到系统 PATH，也跑不到正确版本。
...
<p align="center">
  <img src="docs/screenshots/cli-menu-zh.png" width="420" alt="incipit CLI 主菜单：应用补丁 / 恢复官方 Claude Code / 配置 / 管理目标位置 / CLI 界面语言" />
...
```bash
incipit apply     # 直接应用
incipit restore   # 把当前目标恢复为官方 Claude Code 文件
```

Claude Code 扩展每次更新后，本地补丁会被官方文件覆盖，重跑 `incipit` 并应用即可。更新本 fork 用同一条 GitHub 安装命令：
...
CLI 会先锁定当前 Claude Code 目标，确认后恢复同版本、同扩展目录的官方文件。复制进 `webview/` 的 incipit 资源也会被清掉。你在 VS Code settings 里自己做的其他配置不受影响。

`0.1.7` 过渡期会额外保留 `incipit clean-backups`，用于删除旧版命名备份目录 `~/.incipit-backup/`；它不会删除新的官方恢复点 `~/.incipit/`。
